...
def solve(lines):
    ...
    q = [(0, start[0], start[1], 1, 0, set())]
    best_visited = [[{} for _ in range(len(lines[0]))] for _ in range(len(lines))]
    best_route_tiles = {}
    while len(q) > 0:
        c, x, y, dx, dy, tiles_visited = heapq.heappop(q)
        tiles_visited.add((x, y))
        # print(c, x, y, dx, dy)
        if (x, y) == end:
            best_route_tiles[c] = best_route_tiles.get(c, set()).union(tiles_visited)
            # if c == 7036:
            #     for i, line in enumerate(lines):
            #         for j, c in enumerate(line):
            #             if (j, i) in tiles_visited:
            #                 print("O", end="")
            #             else:
            #                 print(c, end="")
            #         print()
        ...
        for nc, ndx, ndy in [(c+1, dx, dy), (c+1001, lx, ly), (c+1001, rx, ry)]:
            nx, ny = x+ndx, y+ndy
            if lines[ny][nx] != "#":
                heapq.heappush(q, (nc, nx, ny, ndx, ndy, set(tiles_visited)))
    return len(best_route_tiles[min(best_route_tiles.keys())])
